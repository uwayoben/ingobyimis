...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (!["managing_director", "loan_officer", "super_admin"].includes(auth.role))
      return forbidden("Only loan officers and above can add additional management fees.");
    ...
    if (!["active", "overdue", "disbursed"].includes(loan.status))
      return badRequest("Additional management fees can only be added to active or overdue loans.");
    ...
    const updated = await prisma.$transaction(async (tx) => {
      await tx.loanComment.create({
        data: {
          ...
          content:     `[Additional Management Fee] RWF ${amount.toLocaleString()} added. Reason: ${reason}`,
        },
      });

      return tx.loan.update({
        where: { id },
        data: { additionalMgmtFee: { increment: amount } },
      });
    });

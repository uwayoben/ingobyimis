...
// ── More Actions Menu ─────────────────────────────────────────────────────────
function MoreActionsMenu({ items }: { items: { label: string; icon: React.ReactNode; color?: string; danger?: boolean; onClick: () => void }[] }) {
    ...
    <div className="relative">
      ...
      {open && (
        <>
          ...
          <div className="absolute right-0 mt-1 w-52 rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 shadow-lg z-20 overflow-hidden">
            {visible.map((item) => (
              <button
                key={item.label}
                type="button"
                ...
                className={cn(
                  "w-full flex items-center gap-2.5 px-4 py-2.5 text-xs font-medium hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors",
                  item.danger ? "text-red-600 dark:text-red-400" : item.color ?? "text-gray-700 dark:text-gray-300"
                )}
              >
                {item.icon}
                {item.label}
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-start gap-4">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {/* ── Waive Penalty (standalone, only when penalty exists) ── */}
          {canWaive && (
            <Button variant="outline" size="sm" icon={<MinusCircle className="w-4 h-4" />} onClick={() => setShowWaiveModal(true)}>
              Waive Penalty
            </Button>
          )}
          ...
          <MoreActionsMenu items={[
            ...(canTopUp ? [{ label: "Top Up Loan", icon: <TrendingUp className="w-3.5 h-3.5 text-green-500" />, color: "text-green-700 dark:text-green-300", onClick: () => setShowTopUpModal(true) }] : []),
            ...(canAddInterest ? [{ label: "Add Interest",      icon: <TrendingUp className="w-3.5 h-3.5 text-orange-500" />, color: "text-orange-700 dark:text-orange-300", onClick: () => setShowAddInterestModal(true) }] : []),
            ...(canAddMgmtFee  ? [{ label: "Add Mgmt Fee",      icon: <PlusCircle className="w-3.5 h-3.5 text-purple-500" />, color: "text-purple-700 dark:text-purple-300", onClick: () => setShowAddMgmtFeeModal(true) }] : []),

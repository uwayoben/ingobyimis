...
async function main() {
  ...
  const users = await Promise.all([
    // Super admin (no company needed — use ingobyi as placeholder)
    prisma.user.upsert({
  ...
  const [marie, patrick, claudine, samuel, vestine] = customers;
  ...
  const loan1 = await prisma.loan.upsert({
    ...
    create: {
      id: "loan-marie-001",
      customerId: marie.id, companyId: ingobyi.id,
      createdById: alice.id, approvedById: md.id,
      amount: 1000000, interestRate: 2.5, interestType: "declining",
      frequency: "monthly", installments: 12,
      disbursedAmount: 950000, status: "active", purpose: "Business expansion",
      dueDate: new Date("2025-03-05"), nextPaymentDate: new Date("2024-10-05"),
      nextPaymentAmount: 95833, totalRepayable: 1150000, totalPaid: 574998,
      outstandingBalance: 575002, paidInstallments: 6, penaltyAmount: 0,
      approvedAt: new Date("2024-03-03"), disbursedAt: new Date("2024-03-05"),
      fees: { create: [
  ...
  const loan2 = await prisma.loan.upsert({
    ...
    create: {
      id: "loan-patrick-001",
      customerId: patrick.id, companyId: ingobyi.id,
      createdById: alice.id, approvedById: md.id,
      amount: 1500000, interestRate: 3.0, interestType: "flat",
      frequency: "monthly", installments: 18,
      disbursedAmount: 1425000, status: "active", purpose: "Home renovation",
      dueDate: new Date("2025-10-15"), nextPaymentDate: new Date("2024-10-15"),
      nextPaymentAmount: 120000, totalRepayable: 1882500, totalPaid: 600000,
      outstandingBalance: 1282500, paidInstallments: 5, penaltyAmount: 12000,
      approvedAt: new Date("2024-04-12"), disbursedAt: new Date("2024-04-15"),
      fees: { create: [
  ...
  await prisma.loan.upsert({
    ...
    create: {
      id: "loan-samuel-001",
      customerId: samuel.id, companyId: ingobyi.id,
      createdById: alice.id,
      amount: 500000, interestRate: 3.5, interestType: "flat",
      frequency: "monthly", installments: 6,
      disbursedAmount: 0, status: "pending", purpose: "Agricultural inputs",
      dueDate: new Date("2025-03-20"),
      nextPaymentAmount: 97917, totalRepayable: 605000, totalPaid: 0,
      outstandingBalance: 605000, paidInstallments: 0, penaltyAmount: 0,
      fees: { create: [{ name: "Application Fee", type: "fixed", value: 5000, isRecurring: false }] },
    },
  });
  ...
  await Promise.all([
    prisma.payment.upsert({
      ...
      create: {
        ...
        amount: 95833, penalty: 0, interest: 22500, principal: 73333,
  ...
  await prisma.notification.createMany({
    skipDuplicates: true,
    data: [
      { type: "payment_due", title: "Payment Due Tomorrow", isRead: false, companyId: ingobyi.id, link: `/loans/${loan2.id}`, message: "Patrick Nzabonimpa's payment of RWF 120,000 is due on 2024-10-15" },
      ...
      { type: "overdue", title: "Overdue Payment Alert", isRead: true, companyId: ingobyi.id, link: `/loans/${loan2.id}`, message: "Patrick Nzabonimpa has an overdue payment with penalty of RWF 12,000" },
